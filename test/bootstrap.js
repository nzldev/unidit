...
const oldI18n = Unidit.prototype.i18n,
	oldAjaxSender = Unidit.modules.Ajax.prototype.send,
...
function mockAjax() {
	if (typeof window.chai !== 'undefined') {
		let temp = {};
		Unidit.modules.Ajax.prototype.send = function () {
			const ajax = this;
				...
				switch (action) {
					...
					case 'fileUpload': {
						...
						resolve({
							...
							data: {
								baseurl: 'https://nazrul.dev/unidit/files/',
								messages: [],
								files: [file.name],
					...
					case 'getLocalFileByUrl':
						switch (ajax.options.data.url) {
							...
							case location.protocol +
								'//' +
								location.host +
								'/unidit/test/tests/artio.jpg':
							case 'https://nazrul.dev/unidit/files/th.jpg':
								resolve({
									success: true,
...
function unmockAjax() {
	Unidit.modules.Ajax.prototype.send = oldAjaxSender;
}
...
Unidit.prototype.i18n = function (key) {
...
Unidit.defaultOptions.events.afterInit = function (editor) {
	editor &&
		editor.container.setAttribute('data-test-case', window.mochaTestName);
};
Unidit.defaultOptions.filebrowser.saveStateInStorage = false;
Unidit.defaultOptions.observer.timeout = 0;
Unidit.defaultOptions.filebrowser.defaultTimeout = 0;
Unidit.modules.View.defaultOptions.defaultTimeout = 0;

if (Unidit.defaultOptions.cleanHTML) {
	Unidit.defaultOptions.cleanHTML.timeout = 0;
	Unidit.defaultOptions.cleanHTML.fillEmptyParagraph = false;
}

Unidit.defaultOptions.resizer.forImageChangeAttributes = false;

Unidit.defaultOptions.sourceEditor = 'area';
Unidit.defaultOptions.language = 'en';
Unidit.defaultOptions.iframeCSSLinks.push('/app.css');
Unidit.defaultOptions.iframeStyle +=
	'* {\
...
const stringify = Unidit.ns.Helpers.stringify;
...
function removeStuff() {
	Object.keys(Unidit.instances).forEach(function (key) {
		Unidit.instances[key].destruct();
	});
	...
	Array.from(
		document.querySelectorAll(
			'.unidit.unidit-dialog.unidit-dialog_active_true'
		)
	).forEach(function (dialog) {
		simulateEvent('close_dialog', 0, dialog);
	});

	Unidit.modules.Ajax.log.length = 0;
 ...
 * @return {Unidit}
 */
function getUnidit(options, element) {
	const editor = new Unidit(element || appendTestArea(), options);

	window.scrollTo(
		0,
		Unidit.modules.Helpers.offset(editor.container, editor, editor.od).top
	);
		...
		.map(function (elm) {
			...
			if (/rgb\(/.test(keyValue[1])) {
				keyValue[1] = keyValue[1].replace(
					/rgb\([^)]+\)/,
					function (match) {
						return Unidit.modules.Helpers.normalizeColor(match);
					}
				);
			}
			...
			if (/font-family/i.test(keyValue[0])) {
				keyValue[1] = keyValue[1]
					.split(',')
					.map(Unidit.modules.Helpers.trim)
					.map(function (value) {
...
function sortAttributes(html) {
	...
	return html
		.replace(/&nbsp;/g, ' ')
		.replace(Unidit.INVISIBLE_SPACE_REG_EXP(), '');
}
 ...
 * @param {string|number|HTMLElement|Unidit} keyCodeOrElement
 * @param {HTMLElement|Unidit} [element]
...
function simulateEvent(type, keyCodeOrElement, elementOrApplyOpt, applyOpt) {
	...
	if (applyOpt) {
		...
		const pos = Unidit.modules.Helpers.position(element);
...
function getOpenedDialog(editor) {
	const dlgs = editor.ownerDocument.querySelectorAll('.unidit-dialog');
 ...
 * @param {Unidit|Element} uniditOrElement
 * @param {string} [role]
 * @param {boolean} [last]
 * @returns {HTMLElement|null}
 */
function getButton(buttonName, uniditOrElement, role, last) {
	const elm = uniditOrElement.container || uniditOrElement;

	return (
		elm.querySelector(
			'.unidit-toolbar-button.unidit-toolbar-button_' +
				buttonName +
				(last ? ':last-child' : '') +
				' [role="' +
				(role || 'button') +
		...
		elm.querySelector(
			'.unidit-ui-button.unidit-ui-button_' +
				buttonName +
				(last ? ':last-child' : '') +
				'[role="' +
				(role || 'button') +
 ...
 *
 * @param {string} buttonName
 * @param {Unidit|HTMLElement} uniditOrElement
 * @param {string} [role]
 * @param {boolean} [last]
 */
function clickButton(buttonName, uniditOrElement, role, last) {
	simulateEvent(
		['mousedown', 'mouseup', 'click'],
		getButton(buttonName, uniditOrElement, role, last)
	);
}

function clickTrigger(buttonName, uniditOrElement) {
	clickButton(buttonName, uniditOrElement, 'trigger');
}
...
function setCursorToChar(editor, char = '|') {
	...
	Unidit.modules.Dom.each(editor.editor, function (node) {
...
function replaceCursorToChar(editor, char = '|') {
	...
	editor.s.markers.forEach(m => {
		Unidit.modules.Dom.after(m, editor.createInside.text(char));
	});
