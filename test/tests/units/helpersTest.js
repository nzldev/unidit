...
describe('Test helpers', function () {
			...
			it('Should convert some hotkeys to normal', function () {
				...
				Object.keys(hotkeys).forEach(function (key) {
					expect(hotkeys[key]).equals(
						Unidit.modules.Helpers.normalizeKeyAliases(key)
					);
				});
			});
		});
		...
		describe('normalizePath', function () {
			it('Should normalize slashes and join some parts', function () {
				const variants = {
					...
					'data/test': ['data', 'test', ''],
					'test/test/': ['test//test//'],
					'https://nazrul.dev/unidit/connector/index.html': [
						'https://nazrul.dev',
						'unidit/connector/',
						'/index.html'
					],
					'https://nazrul.dev/unidit/connector/index2.html': [
						'https://nazrul.dev\\unidit/connector/',
						'/index2.html'
					]
				};
				...
				Object.keys(variants).forEach(function (key) {
					expect(key).equals(
						Unidit.modules.Helpers.normalizePath.apply(
							null,
							variants[key]
						)
					);
	...
	describe('Checkers', function () {
			...
			it('Should check value is undefned or null', function () {
				...
				for (let i = 0; i < values.length; i += 1) {
					...
					expect(value[1]).equals(
						Unidit.modules.Helpers.isVoid(value[0])
					);
				}
			});
		});
			...
			it('Should check value is URL', function () {
				...
				for (let i = 0; i < values.length; i += 1) {
					...
					expect(result).equals(
						Unidit.modules.Helpers.isURL(key),
						`Is domain "${key}"`
					);
				}
			});
			...
			it('Should check value is int or not', function () {
				...
				for (let i = 0; i < values.length; i += 2) {
					expect(values[i + 1]).equals(
						Unidit.modules.Helpers.isInt(values[i])
					);
				}
			});
		});
		...
		describe('isNumeric', function () {
			it('Should check value is int or not', function () {
				...
				for (let i = 0; i < values.length; i += 2) {
					expect(values[i + 1]).equals(
						Unidit.modules.Helpers.isNumeric(values[i])
					);
				}
			});
		});
		...
		describe('isNumber', function () {
			it('Should check value is a number', function () {
				...
				for (let i = 0; i < values.length; i += 2) {
					expect(values[i + 1]).equals(
						Unidit.modules.Helpers.isNumber(values[i])
					);
				}
			});
		});
	...
	describe('String', function () {
		describe('18n', function () {
			const i18n = Unidit.modules.Helpers.i18n;
		...
		describe('stripTags', function () {
			describe('Put HTML text', function () {
				it('Should return only text', function () {
					...
					for (let i = 0; i < values.length; i += 1) {
						expect(values[i][1]).equals(
							Unidit.modules.Helpers.stripTags(
								values[i][0]
							).replace(/\n/g, '')
						);
					}
	...
	describe('Object', function () {
		describe('get', function () {
			it('Should get value from keyChain else return null', function () {
				...
				for (let i = 0; i < values.length; i += 1) {
					const value = values[i];
					expect(value[1]).equals(
						Unidit.modules.Helpers.get(value[0], obj)
					);
				}
			});
		});
		...
		describe('set', function () {
			it('Should set value by keyChain', function () {
				...
				for (let i = 0; i < values.length; i += 1) {
					const value = values[i];
					Unidit.modules.Helpers.set(value[0], value[1], obj);
	...
	describe('Utils', function () {
		describe('reset', function () {
			it('It should reset native browser method', function () {
				expect(typeof Unidit.modules.Helpers.reset('Array.from')).equals(
					'function'
				);

				expect(Unidit.modules.Helpers.reset('Array.from') !== Array.from)
					.is.true;

				expect(
					Unidit.modules.Helpers.reset('Array.from')(
						new Set([1, 2, 3])
					)
				).deep.equals([1, 2, 3]);

				expect(
					Unidit.modules.Helpers.reset('Array.from')('123')
				).deep.equals(['1', '2', '3']);
			});
		...
		describe('getClassName', function () {
			const getClassName = Unidit.modules.Helpers.getClassName;

			it('Should return normal(not uglifyed) name for instance of class', function () {
				expect(getClassName(Unidit.modules.Popup.prototype)).equals(
					'Popup'
				);
				expect(getClassName(Unidit.modules.UIButton.prototype)).equals(
					'UIButton'
				);
				expect(
					getClassName(Unidit.modules.ToolbarButton.prototype)
	...
	describe('Config prototype', function () {
		const ConfigProto = Unidit.modules.Helpers.ConfigProto;
