			...
			Unidit.make(area, {
				...
				events: {
					afterConstructor: function (unidit) {
						expect(
							unidit.editor.getAttribute('secret-attribute')
						).equals('435'); // loaded from index.html
						expect(Unidit.ns.Helpers.trim(unidit.value)).equals(
							'<p>test 435</p>'
					...
					['generateDocumentStructure.iframe']: function (
						...
						unidit.events.stopPropagation(
							'generateDocumentStructure.iframe'
						);
						return new Promise(resolve => {
							unidit.iframe.onload = function () {
								resolve();
							};
							...
							unidit.iframe.src = 'test.index.html';
						});
					}
				}
			});
