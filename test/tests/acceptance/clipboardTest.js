	...
	describe('Paste HTML', function () {
		it('Should show paste html dialog', function () {
			const editor = getUnidit({
				defaultActionOnPaste: Unidit.INSERT_AS_HTML
			});
			...
			describe('Insert only text', function () {
				it('Should insert only text from pasted html', function () {
					const editor = getUnidit({
							...
							defaultActionOnPaste: Unidit.INSERT_ONLY_TEXT
						}),
						pastedText = '<p>test</p>',
				...
				describe('For not empty editor', function () {
					it('Should insert text on the cursor place', function () {
						// https://github.com/nzldev/unidit/issues/522
						const editor = getUnidit({
								askBeforePasteHTML: false,
								askBeforePasteFromWord: false,
								defaultActionOnPaste: Unidit.INSERT_ONLY_TEXT
						...
						editor.value = '<p>Unidit is awesome |</p>';
			...
			describe('Insert as text', function () {
				it('Should insert only text from pasted html', function () {
					const editor = getUnidit({
						askBeforePasteHTML: false,
						askBeforePasteFromWord: false,
						defaultActionOnPaste: Unidit.INSERT_AS_TEXT
					});
					const pastedText = '<p>test</p>';
			...
			describe('Insert clear html', function () {
				it('Should insert pasted and cleared html', function () {
					const editor = getUnidit({
						askBeforePasteHTML: false,
						askBeforePasteFromWord: false,
						defaultActionOnPaste: Unidit.INSERT_CLEAR_HTML
					});
	...
	describe('Cut and Copy', function () {
		describe('After cut or copy commands', function () {
			['copy', 'cut'].forEach(function (command) {
				describe(
					'For ' + command + ' command. In Unidit.buffer',
					function () {
