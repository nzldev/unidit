 ...
 */
describe('Commands Unidit Editor Tests', function () {
	describe('Command "formatBlock"', function () {
		...
		describe('Exec formatBlock for one inline element', function () {
			it('Should wrap this element and all nearest inline element in block', function () {
				...
				unidit.value = 'stop <span>post</span> ice';
				const range = unidit.ed.createRange();
				range.setStart(unidit.editor.firstChild, 0);
				range.setEnd(unidit.editor.firstChild, 2);
				unidit.s.selectRange(range);

				unidit.execCommand('formatBlock', false, 'h1');
