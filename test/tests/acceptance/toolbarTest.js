...
describe('Toolbar', function () {
	it('Should have buttons', function () {
		...
		expect(
			editor.toolbar.container.querySelectorAll('.unidit-toolbar-button')
				.length
		).equals(37);
	});
	...
	describe('Custom buttons', function () {
		it('should create normal button in toolbar', function () {
			const editor = getUnidit({
				...
				buttons: [
					'image',
					{
						name: 'alert_some',
						iconURL:
							'https://nazrul.dev/unidit/build/images/icons/045-copy.png',
						exec: function () {
							alert('test');
						}
					}
			...
			btns.forEach(function (btn) {
				const icon = btn.querySelector('.unidit-icon');
	...
	describe('Custom icons', function () {
		describe('By default for unknown icons', function () {
			it('should create button as text mode', function () {
				...
				expect(
					...
					'<span class="unidit-toolbar-button__icon"></span><span class="unidit-toolbar-button__text">rect</span>'
				);
			});
		});
		...
		describe('Use getIcon event', function () {
			it('should create buttons with custom icons', function () {
				...
				expect(
					...
					'<span class="unidit-toolbar-button__icon">' +
						'<i class="fa fa-image fa-xs unidit-icon_image unidit-icon" style="font-size:14px"></i>' +
						'</span><span class="unidit-toolbar-button__text"></span>'
				);
				...
				expect(
					...
					'<span class="unidit-toolbar-button__icon">' +
						'<i class="fa fa-rotate-right fa-xs unidit-icon_redo unidit-icon" style="font-size:14px"></i>' +
						'</span><span class="unidit-toolbar-button__text"></span>'
				);
				...
				expect(
					...
					'<span class="unidit-toolbar-button__icon">' +
						'<i class="fa fa-question fa-xs unidit-icon_about unidit-icon" style="font-size:14px"></i>' +
						'</span><span class="unidit-toolbar-button__text"></span>'
				);
			});
		});
		...
		const rect =
			...
			rectResult =
				'<span class="unidit-toolbar-button__icon">' +
				'<svg class="unidit-icon_rect unidit-icon" height="110" width="400"><rect height="100" width="300"></rect></svg>' +
				'</span><span class="unidit-toolbar-button__text"></span>';
		...
		describe('Use Icon module', function () {
			it('should create buttons with custom icons', function () {
				Unidit.modules.Icon.set('rect', rect);
				const editor = getUnidit({
					toolbarAdaptive: false,
					buttons: [
						{
		...
		describe('Use svg in icon', function () {
			it('should create buttons with custom icons', function () {
				...
				expect(
					sortAttributes(getButton('rect', editor).innerHTML)
				).equals(rectResult.replace('unidit-icon_rect ', ''));
			});
		});
	});
	...
	describe('Set toolbar options to false', function () {
		it('Should hide toolbar', function () {
			...
			expect(null).equals(
				editor.container.querySelector('.unidit_toolbar')
			);
		});
	});
	...
	describe('Set toolbar options to css selector', function () {
		it('Should render toolbar in different container', function () {
			...
			const toolbar = document.querySelector(
				'.unidit-toolbar-editor-collection'
			);
		...
		describe('After enable Fullsize mode', function () {
			it('Should render toolbar in default container', function () {
				const div = appendTestDiv(),
					editor = Unidit.make(appendTestArea(), {
						toolbar: div
					});
	...
	describe('Popups', function () {
		...
		it('Open video dialog and insert video by url from youtube.', function () {
			...
			simulateEvent('submit', popup.querySelector('.unidit-ui-form'));

			expect(
				popup.querySelectorAll(
					'.unidit-ui-input.unidit-ui-input_has-error_true'
				).length
			).equals(1);
		...
		describe('Create table', function () {
			describe('Mouse move', function () {
				it('Should highlight cells in table-creator', function () {
					...
					simulateEvent(
						'mousemove',
						list.querySelectorAll('.unidit-form__container span')[14]
					);

					expect(
						list.querySelectorAll(
							'.unidit-form__container span.unidit_hovered'
						).length
					).equals(10);
				});
	...
	describe('Buttons', function () {
		...
		describe('Text mode', function () {
			...
			it("Should add unidit_text_icons class to editor's container", function () {
				const editor = getUnidit({
					textIcons: true
				});
		...
		describe('Set empty Unidit.atom', function () {
		...
		it('Add own button', function () {
			const editor = getUnidit({
				disablePlugins: ['mobile'],
				buttons: Unidit.defaultOptions.buttons.concat([
					{
						name: 'insertDate',
						iconURL: 'http://nazrul.dev/pacakges/unidit/images/logo.png',
		...
		describe('Disable for mode', function () {
			it('Should disable buttons which can not be used in that mode', function () {
				...
				editor.setMode(Unidit.MODE_WYSIWYG);
			...
			describe('For list', function () {
				describe('enable', function () {
					it('Should enable buttons which can be used in this mode', function () {
						...
						expect(
							list.querySelectorAll('.unidit_disabled').length
						).equals(0);
					});
				});
				...
				describe('disable', function () {
					it('Should disable buttons which can not be used in that mode', function () {
						...
						const list = btn.querySelector('.unidit_toolbar_list');
						expect(list).is.null;
					});
				});
			});
			...
			it('Should add modification to button', function () {
				...
				expect(
					getButton(
						...
						'unidit-toolbar-button_size_small'
					)
				).is.true;
			});
		...
		it('Full size button', function () {
			...
			while (node && node.nodeType !== Node.DOCUMENT_NODE) {
				expect(
					node.classList.contains('unidit_fullsize-box_true')
				).equals(true);
	...
	describe('Commands', function () {
		it('Click on Source button should change current mode', function () {
			...
			expect(editor.getMode()).equals(Unidit.MODE_SOURCE);
		});
			...
			it('should hide toolbar buttons', function () {
				...
				expect(3).equals(
					dialog.querySelectorAll(
						'.unidit-dialog__header .unidit-dialog__header-title button,' +
							'.unidit-dialog__header .unidit-dialog__header-title select'
					).length
				);
			});
		});
	...
	describe('One toolbar for several editors', function () {
		it('Should create one Unidit instance but with several edit places', function () {
			const toolbarBox = appendTestDiv(),
				...
				editor = Unidit.make(firstEditPlace);
			...
			expect(Unidit.instances.firstEditPlace).equals(editor);
			expect(Unidit.instances.secondEditPlace).equals(undefined);
			expect(Unidit.instances.thirdEditPlace).equals(undefined);
		...
		describe('For all instances you can set self options', function () {
			it('Should change options for all instances', function () {
				...
				const editPlaces =
					editor.ownerDocument.querySelectorAll('.unidit-wysiwyg');
