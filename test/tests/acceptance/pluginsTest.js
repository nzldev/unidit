...
describe('Test plugins', function () {
	...
	describe('Add new Line plugin', function () {
		it('Should not add new line element in container before first use', function () {
			const editor = getUnidit();
			expect(
				editor.container.querySelectorAll('.unidit-add-new-line').length
			).equals(0);
		});

		const moveCursorUnder = function (editor, elm) {
			simulateEvent('mousemove', 0, editor.editor, function (e) {
				const pos = Unidit.modules.Helpers.position(elm, editor);
		...
		it('Should show .unidit-add-new-line after user move mouse under Table,Ifrmae or IMG ', function () {
			const editor = getUnidit();
			editor.value =
				'<table>' +
				'<tbody>' +
		...
		it('Should add new paragraph after user clicked on newline below table', function () {
			...
			simulateEvent('mousemove', 0, editor.editor, function (data) {
				const pos = Unidit.modules.Helpers.position(
				...
				describe('Enable ratio', function () {
					it('Should deny crop image without ratio', function (done) {
						...
						const editor = new Unidit(area, {
							...
							uploader: {
								url: 'https://nazrul.dev/unidit/connector/index.php?action=upload'
							},
							filebrowser: {
								ajax: {
						...
						editor.value =
							'<img alt="" src="https://nazrul.dev/unidit/files/th.jpg">';
						...
						editor.filebrowser.events.on(
							...
							function () {
								...
								expect(
									imageEditor.querySelectorAll(
										'[data-area=crop].unidit-image-editor_active'
									).length
								).equals(0);
								...
								simulateEvent(
									'mousedown',
									0,
									cropper.querySelector('.unidit_bottomright'),
									function (e) {
										const pos =
											Unidit.modules.Helpers.offset(
												cropper,
					...
					it('Should deny resize image without ratio', function (done) {
						...
						editor.filebrowser.events.on(
							...
							function () {
								...
								expect(
									imageEditor.querySelectorAll(
										'[data-area=resize].unidit-image-editor_active'
									).length
								...
								simulateEvent(
									'mousedown',
									0,
									resizer.querySelector('.unidit_bottomright'),
									function (e) {
										const pos =
											Unidit.modules.Helpers.offset(
												resizer,
		...
		describe('Symbols dialog', function () {
			...
			describe('Press Enter or mousdown on element', function () {
				it('Should insert character', function () {
					...
					simulateEvent('keydown', Unidit.KEY_ENTER, currentActive);
	...
	describe('Sticky plugin', function () {
		describe('Without scrolling', function () {
			it('Should not have `unidit_sticky` class and toolbar must be in normal state', function () {
				const area = appendTestArea(),
					editor = new Unidit(area);

				editor.value = '<p>stop</p>'.repeat(100);
				expect(false).equals(
					editor.container.classList.contains('unidit_sticky')
				);
			});
		});
		...
		describe('Create editor in page with long text', function () {
			describe('and scroll page to bottom', function () {
				it('Should add to editor class `unidit_sticky` and toolbar must be always on the top', function () {
					const editor = getUnidit();

					editor.value = '<p>stop</p>'.repeat(100);

					const offset = Unidit.modules.Helpers.offset(
						editor.container,
						editor,
						editor.ownerDocument
					);
				...
				describe('On mobile devices - with toolbarDisableStickyForMobile = true', function () {
					it('Should not add to editor class `unidit_sticky`', function () {
			...
			describe('and scroll page to the top', function () {
				it('Should remove class `unidit_sticky` from editor and toolbar must have normal position', function () {
					fillBoxBr(100);
					...
					expect(5).to.be.above(
						Math.abs(
							200 -
								Unidit.modules.Helpers.position(
		...
		describe('After init', function () {
			describe('With showXPathInStatusbar=true', function () {
				it('Should show status bar', function () {
					...
					const statusbar =
						editor.container.querySelector('.unidit-status-bar');
				...
				it('Should show path to selection element', function () {
					...
					const statusbar = editor.container.querySelector(
						'.unidit-status-bar .unidit-xpath'
					);
				...
				describe('After click on element of path', function () {
					it('Should select this element', function () {
						...
						expect(
							Unidit.modules.Helpers.trim(editor.s.sel.toString())
						).equals('sss');
						expect(statusbar.childNodes[2].textContent).equals('a');
				...
				describe('Context menu on element of path', function () {
					it('Should open context menu', function () {
						...
						const elm = statusbar.childNodes[2].querySelector('a'),
							pos = Unidit.modules.Helpers.position(elm);
			...
			describe('After click on some of elements', function () {
				it('Sholud select this', function () {
					...
					simulateEvent(
						'click',
						0,
						dialog.querySelectorAll('.unidit-paste-storage a')[1]
					);

					expect(
						dialog
							.querySelectorAll('.unidit-paste-storage a')[1]
							.classList.contains('unidit_active')
					).is.true;
			...
			describe('Press key up/down/enter', function () {
				it('Sholud select next/previos element of list and insert selected value after Enter', function () {
					...
					simulateEvent(
						'click',
						0,
						dialog.querySelectorAll('.unidit-paste-storage a')[0]
					);
					expect(
						dialog
							.querySelectorAll('.unidit-paste-storage a')[0]
							.classList.contains('unidit_active')
					).is.true;
