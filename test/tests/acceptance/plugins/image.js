	...
	describe('Toolbar', function () {
		describe('Click on Image button', function () {
			it('Should open image dialog and insert image by url.', function () {
				...
				editor.value = Unidit.INVISIBLE_SPACE; // IE in iframe mode can loose focus and we can not check where it paste image in start or in finish. It is only in IE
				...
				expect(
					list.querySelectorAll('.unidit-ui-input_has-error_true')
						.length
				).equals(1);

				list.querySelector('input[name=url]').value =
					'http://nazrul.dev/pacakges/unidit/images/artio.jpg';

				simulateEvent('submit', 0, list.querySelector('form'));

				expect(sortAttributes(editor.value)).equals(
					'<p><img alt="123" src="http://nazrul.dev/pacakges/unidit/images/artio.jpg" style="width:300px"></p>'
				);
			...
			describe('When the cursor in the middle of some text', function () {
				it('Should insert image in this position after submit', function () {
					...
					input.focus();
					input.value = 'http://nazrul.dev/pacakges/unidit/images/artio.jpg';

					simulateEvent('submit', 0, list.querySelector('form'));

					expect(sortAttributes(editor.value)).equals(
						'<p>hello<img alt="" src="http://nazrul.dev/pacakges/unidit/images/artio.jpg" style="width:300px"> world!</p>'
					);
				});
			});
		});
