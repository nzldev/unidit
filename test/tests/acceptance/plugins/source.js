			...
			it('Should shoe source and wysiwyg in same time', function () {
				...
				expect(
					editor.ew.getComputedStyle(
						editor.container.querySelector('.unidit-source')
					).display
				).equals('block');
			}).timeout(6000);
		});
				...
				getUnidit({
					...
					events: {
						sourceEditorReady: function (unidit) {
							unidit.s.focus();
							unidit.value = '<p>test <span>test</span> test</p>';
							const range = unidit.ed.createRange();

							range.selectNodeContents(
								unidit.editor.querySelector('span')
							);
							range.collapse(false);

							unidit.s.selectRange(range);

							unidit.setMode(Unidit.MODE_SOURCE);

							unidit.s.insertHTML('loop');

							expect(unidit.value).equals(
