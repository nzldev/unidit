...
export class Config implements IViewOptions {
	...
	/**
	 * Shadow root if Unidit was created in it
	 */
	shadowRoot: Nullable<ShadowRoot> = null;

	/**
	 * Dictionary of variable values in css, a complete list can be found here
	 * https://github.com/nzldev/unidit/blob/master/src/styles/variables.less#L25
	 *
	 * @example
	 * ```js
	 ...
	 * @example
	 * ```javascript
	 * var editor  = new  Unidit(".dark_editor", {
	 ...
	 *
	 * @see {@link Unidit.defaultOptions.iframeCSSLinks|iframeCSSLinks}
	 * @see {@link Unidit.defaultOptions.iframeStyle|iframeStyle}
	 *
	 * @example
	 * ```javascript
	 * new Unidit('#editor', {
	 ...
	 * <script src="unidit/lang/de.js"></script>
	 * <script>
	 * var editor = new Unidit('.editor', {
	 *    language: 'de'
	 * });
	...
	/**
	 * Unidit.MODE_WYSIWYG The HTML editor allows you to write like MSWord,
	 * Unidit.MODE_SOURCE syntax highlighting source editor
	 * @example
	 * ```javascript
	 * var editor = new Unidit('#editor', {
	 *     defaultMode: Unidit.MODE_SPLIT
	 * });
	...
	/**
	 * By default, you can only install an icon from the Unidit suite.
	 * You can add your icon to the set using the `Unidit.modules.Icon.set (name, svg Code)` method.
	 * But for a declarative declaration, you can use this option.
	 *
	 * @example
	 * ```js
	 * Unidit.modules.Icon.set('someIcon', '<svg><path.../></svg>');
	 * const editor = Unidit.make({
	 *   extraButtons: [{
	 *     name: 'someButton',
	 *     icon: 'someIcon'
	 *   }]
	 ...
	 *         icon: 'source',
	 *         exec: function () {
	 *             var dialog = new Unidit.modules.Dialog(this),
