...
interface IControlType<
	T = IUnidit | IViewBased | IFileBrowser,
	B = IToolbarButton
> {
	name?: string;
	text?: string;
	 ...
	 * @example
	 * ```javascript
	 * var editor = new Unidit('.selectorclass', {
	 *     buttons: {
	 *          checkbox: {
	 *              data: {
	 *                  active: false,
	 ...
	 * (or array value) (see .[[Unidit.execCommand]] or define 'exec' function. See example
	 * @example
	 * ```javascript
	 * new Unidit('#editor2', {
	 ...
	 * and several specific [[Unidit.execCommand]]
	 */
	command?: string;
	tagRegExp?: RegExp;
	 ...
	 */
	template?: (unidit: T, key: string, value: string) => string;
	childTemplate?: (unidit: T, key: string, value: string) => string;
