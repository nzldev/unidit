...
import { humanSizeToBytes, isArray, isString } from 'unidit/core/helpers';
import { UIFileInput } from 'unidit/core/ui';
...
Config.prototype.filebrowser = {
	...
	getThumbTemplate(
		...
		return `<a
			data-unidit-filebrowser-item="true"
...
Config.prototype.controls.filebrowser = {
	...
	filter: {
		...
		getContent: (filebrowser: IFileBrowser, _, b): HTMLElement => {
			const oldInput = b.container.querySelector('.unidit-input');
	...
	sort: {
		...
		getContent: (fb: IFileBrowser): HTMLElement => {
			const select: HTMLSelectElement = fb.c.fromHTML(
				'<select class="unidit-input unidit-select">' +
