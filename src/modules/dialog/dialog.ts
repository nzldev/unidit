...
import { KEY_ESC } from 'unidit/core/constants';
import {
	$$,
	asArray,
	assert,
...
import { ViewWithToolbar } from 'unidit/core/view/view-with-toolbar';
import { Dom } from 'unidit/core/dom';
import { STATUSES } from 'unidit/core/component';
import { eventEmitter, pluginSystem } from 'unidit/core/global';
import { component, autobind, hook } from 'unidit/core/decorators';
import { View } from 'unidit/core/view/view';
...
export class Dialog extends ViewWithToolbar implements IDialog {
	 ...
	 * dialog.setFooter([
	 *  $('<a class="unidit-button">OK</a>').click(function () {
	 *      alert($('someText').val())
	 *      dialog.close();
	 *  })
	 * ]);
	...
	getMaxZIndexDialog(): IDialog {
		...
		$$('.unidit-dialog', this.destination).forEach((dialog: HTMLElement) => {
	...
	setMaxZIndex(): void {
		...
		$$('.unidit-dialog', this.destination).forEach(dialog => {
			zIndex = parseInt(css(dialog, 'zIndex') as string, 10);
	 ...
	 * var dialog = new Unidit.modules.Dialog();
	 * dialog.open('Hello world!', 'Title');
	 * var $close = dialog.create.fromHTML('<a href="#" style="float:left;" class="unidit-button">
	 *     <i class="icon icon-check"></i>&nbsp;' + Unidit.prototype.i18n('Ok') + '</a>');
	...
	close(e?: MouseEvent): this {
		...
		this.e?.fire(this.ow, 'uniditCloseDialog');
	...
	constructor(options?: Partial<IDialogOptions>) {
		...
		self.container = this.c.fromHTML(
			`<div style="z-index:${self.o.zIndex}" class="unidit unidit-dialog ${
				this.componentName
			}">
