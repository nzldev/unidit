...
import type { ICreate, IDictionary, IUnidit } from 'unidit/types';
...
import { debounce } from 'unidit/core/decorators';
...
export class Table extends ViewComponent<IUnidit> {
	/** @override */
	className(): string {
		return 'Table';
	}
	...
	private recalculateStyles(): void {
		...
		style.innerHTML = selectors.length
			? selectors.join(',') +
			  `{${(this.unidit.options as any).table.selectionCellStyle}}`
			: '';
	}
	 ...
	 */
	static mergeSelected(table: HTMLTableElement, unidit: IUnidit): void {
		const html: string[] = [],
		...
		if (bound && (bound[0][0] - bound[1][0] || bound[0][1] - bound[1][1])) {
					...
					if (i >= bound[0][0] && i <= bound[1][0]) {
						if (j >= bound[0][1] && j <= bound[1][1]) {
							...
							if (!first) {
								...
								instance(unidit).removeSelection(td);
							}
						}
					}
				}
			...
			if (first) {
				...
				instance(unidit).addSelection(first);
	...
	/**
	 * Divides all selected by `unidit_focused_cell` class table cell in 2 parts vertical. Those division into 2 columns
	 */
	static splitHorizontal(table: HTMLTableElement, unidit: IUnidit): void {
		let coord: number[],
			td: HTMLTableCellElement,
		...
		Table.getSelectedCellsByTable(table).forEach(
			(cell: HTMLTableCellElement) => {
				td = unidit.createInside.element('td');
				td.appendChild(unidit.createInside.element('br'));
				tr = unidit.createInside.element('tr');
				...
				Table.unmark(__marked);
				instance(unidit).removeSelection(cell);
			}
		);
	 ...
	 */
	static splitVertical(table: HTMLTableElement, unidit: IUnidit): void {
...
const instance = (j: IUnidit): Table => j.getInstance<Table>('Table', j.o);
