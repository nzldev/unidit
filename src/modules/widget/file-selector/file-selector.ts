...
import { isFunction, $$, attr, val } from 'unidit/core/helpers';
import { Dom } from 'unidit/core/dom';
import { TabOption, TabsWidget } from '../tabs/tabs';
import { UIBlock, UIForm, UIInput, UIButton } from 'unidit/core/ui';
...
interface ImageSelectorCallbacks {
	 ...
	 */
	url?: (this: IUnidit, url: string, alt: string) => void;
	 ...
	 */
	upload?: ((this: IUnidit, data: IFileBrowserCallBackData) => void) | true;
}
 ...
 * @example
 * ```javascript
 * let widget = new Unidit.modules.Widget(editor);
 *
...
export const FileSelectorWidget = (
	...
	if (
		...
		const dragBox = editor.c.fromHTML(
			'<div class="unidit-drag-and-drop__file-box">' +
				`<strong>${editor.i18n(
