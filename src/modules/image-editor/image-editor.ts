...
import { Alert, Dialog, Prompt } from 'unidit/modules/dialog';
import { $$, attr, css, refs, toArray, trim } from 'unidit/core/helpers';
import { Dom } from 'unidit/core/dom';
import { Button } from 'unidit/core/ui/button';
import { form } from './templates/form';
import { component, debounce, throttle, autobind } from 'unidit/core/decorators';
...
export class ImageEditor extends ViewComponent {
	...
	private setHandlers = () => {
		...
		self.j.e
			.on(
				[
					self.editor.querySelector('.unidit_bottomright'),
					self.cropHandler
				],
				`mousedown.${jie}`,
	 ...
	 *		 }
	 * });
	 * unidit.imageeditor.open('http://nazrul.dev/pacakges/unidit/images/test.png', function (name, data, success, failed) {
	 *		 var img = unidit.node.c('img');
	 *		 img.setAttribute('src', 'http://nazrul.dev/pacakges/unidit/images/test.png');
	...
	open(
		...
		return this.j.async.promise<Dialog>((resolve: Function): void => {
			...
			$$('img,.unidit-icon_loader', this.resize_box).forEach(
				Dom.safeRemove
			);

			$$('img,.unidit-icon_loader', this.crop_box).forEach(Dom.safeRemove);
			...
			this.resize_box.appendChild(
				this.j.c.element('i', { class: 'unidit-icon_loader' })
			);
			...
			const onload = () => {
				...
				$$('.unidit-icon_loader', this.editor).forEach(Dom.safeRemove);
	...
	constructor(editor: IViewBased) {
		...
		this.options =
			editor && (editor as IUnidit).o && (editor as IUnidit).o.imageeditor
				? (editor as IUnidit).o.imageeditor
		...
		this.sizes = this.editor.querySelector(
			`.${jie}__area.${jie}__area_crop .unidit-image-editor__sizes`
		) as HTMLElement;
