...
import { IS_IE, TEXT_PLAIN } from 'unidit/core/constants';
import { Ajax } from 'unidit/core/request';
import {
	attr,
	error,
	isPlainObject,
...
export class Uploader extends ViewComponent implements IUploader {
	 ...
	 * Unidit.editors.someidfoeditor.uploader.bind($form[0], function (files) {
	 *     var i;
	...
	bind(
			...
			onPaste = (e: ClipboardEvent): false | void => {
				...
				if (IS_IE && !isESNext) {
					if (
						...
						const selection = isUniditObject(this.j)
								? this.j.s.save()
								: null,
							restore = () =>
								selection &&
								isUniditObject(this.j) &&
								this.j.s.restore();
		...
		if (this.j && (this.j as IUnidit).editor !== form) {
		...
		self.j.e
			...
			.on(form, 'dragover', (event: DragEvent) => {
				if (hasFiles(event.dataTransfer)) {
					form.classList.contains('unidit_draghover') ||
						form.classList.add('unidit_draghover');
			...
			.on(form, 'dragend', (event: DragEvent) => {
				if (hasFiles(event.dataTransfer)) {
					form.classList.contains('unidit_draghover') &&
						form.classList.remove('unidit_draghover');
					event.preventDefault();
				}
			})
	...
	constructor(editor: IViewBased, options?: IUploaderOptions<Uploader>) {
		...
		this.options = ConfigProto(
			...
			ConfigProto(
				...
				isUniditObject(editor) ? editor.o.uploader : {}
			)
		) as IUploaderOptions<Uploader>;
	}
