...
import { css, attr } from 'unidit/core/helpers';
import { Dom } from 'unidit/core/dom';
import { Plugin } from 'unidit/core/plugin';
import { MAY_BE_REMOVED_WITH_KEY } from 'unidit/core/constants';
...
export class placeholder extends Plugin {
	...
	protected afterInit(editor: IUnidit): void {
		...
		this.placeholderElm = editor.c.fromHTML(
			`<span data-ref="placeholder" style="display: none;" class="unidit-placeholder">${editor.i18n(
	...
	protected beforeDestruct(unidit: IUnidit): void {
		...
		unidit.e.off('.placeholder').off(window, 'load', this.toggle);
	}
}
