...
import type { IControlTypeStrong, IToolbarButton } from 'unidit/types';
import { Config } from 'unidit/config';
import { INVISIBLE_SPACE, MODE_WYSIWYG } from 'unidit/core/constants';
import { ContextMenu } from 'unidit/modules/context-menu/context-menu';
import { Dom } from 'unidit/core/dom';
import { getXPathByElement, trim, attr } from 'unidit/core/helpers';
import { Plugin } from 'unidit/core/plugin';
import { makeButton } from 'unidit/modules/toolbar/factory';
...
export class xpath extends Plugin {
	...
	private tpl = (
		...
		const item = this.j.c.fromHTML(
			`<span class="unidit-xpath__item"><a role="button" data-path="${path}" title="${title}" tabindex="-1"'>${trim(
				name
			)}</a></span>`
		) as HTMLLIElement;
	...
	afterInit(): void {
		if (this.j.o.showXPathInStatusbar) {
			this.container = this.j.c.div('unidit-xpath');
