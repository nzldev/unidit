...
import type { IUnidit, ISourceEditor } from 'unidit/types';
import * as consts from 'unidit/core/constants';
import { INVISIBLE_SPACE, MODE_SOURCE, MODE_SPLIT } from 'unidit/core/constants';
import { Plugin } from 'unidit/core/plugin';
import { Dom } from 'unidit/core/dom';
import { isString, loadNext } from 'unidit/core/helpers';
...
export class source extends Plugin {
	...
	private tempMarkerStart = '{start-unidit-selection}';
	private tempMarkerStartReg = /{start-unidit-selection}/g;
	private tempMarkerEnd = '{end-unidit-selection}';
	private tempMarkerEndReg = /{end-unidit-selection}/g;
	...
	protected removeSelection(): void {
		...
		try {
			value = value
				.replace(
					/<span[^>]+data-unidit-selection_marker=(["'])start\1[^>]*>[<>]*?<\/span>/gim,
					this.tempMarkerStart
				)
				.replace(
					/<span[^>]+data-unidit-selection_marker=(["'])end\1[^>]*>[<>]*?<\/span>/gim,
					this.tempMarkerEnd
				);
	...
	afterInit(editor: IUnidit): void {
		this.mirrorContainer = editor.c.div('unidit-source');
	...
	private initSourceEditor(editor: IUnidit): void {
