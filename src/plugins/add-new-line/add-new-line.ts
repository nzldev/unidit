...
import type { IBound, IUnidit, HTMLTagNames, Nullable } from 'unidit/types';
import { Config } from 'unidit/config';
import { Dom, Icon, Plugin } from 'unidit/modules';
import {
	offset,
	position,
	call,
...
/**
 * Create helper for adding new paragraph(Unidit.defaultOptions.enter tag) before iframe, table or image
 */
export class addNewLine extends Plugin {
	private line = this.j.c.fromHTML(
		`<div role="button" tabindex="-1" title="${this.j.i18n(
			'Break'
		)}" class="unidit-add-new-line"><span>${Icon.get('enter')}</span></div>`
	) as HTMLDivElement;
	...
	private show() {
		...
		this.line.classList.toggle('unidit-add-new-line_after', !this.preview);
