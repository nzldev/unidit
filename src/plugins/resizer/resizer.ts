...
import type { HTMLTagNames, IBound, Nullable } from 'unidit/types';
...
import { eventEmitter } from 'unidit/core/global';
import { autobind, debounce, watch } from 'unidit/core/decorators';
...
Config.prototype.allowResizeTags = ['img', 'iframe', 'table', 'unidit'];
...
const keyBInd = '__unidit-resizer_binded';
...
export class resizer extends Plugin {
	...
	private rect = this.j.c.fromHTML(
		`<div class="unidit-resizer">
				<div class="unidit-resizer__top-left"></div>
				<div class="unidit-resizer__top-right"></div>
				<div class="unidit-resizer__bottom-right"></div>
				<div class="unidit-resizer__bottom-left"></div>
				<span>100x100</span>
			</div>`
	);
	...
	/** @override */
	protected afterInit(editor: IUnidit): void {
		...
		editor.e
			...
			.on(
				...
				(data: { value: string }) => {
					const rgx =
						/<unidit[^>]+data-unidit_iframe_wrapper[^>]+>(.*?<iframe[^>]*>.*?<\/iframe>.*?)<\/unidit>/gi;
	...
	private onDelete(e: KeyboardEvent) {
		...
		if (this.element.tagName !== 'UNIDIT') {
	...
	private bind(element: HTMLElement): void {
		...
		if (Dom.isTag(element, 'iframe')) {
			...
			if (
				...
				attr(element.parentNode, '-unidit_iframe_wrapper')
			) {
	...
	protected beforeDestruct(unidit: IUnidit): void {
		...
		unidit.e.off(this.j.ow, '.resizer').off('.resizer');
	}
}
