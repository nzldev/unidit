...
import { MODE_WYSIWYG } from 'unidit/core/constants';
import { Dom } from 'unidit/core/dom';
import { Plugin } from 'unidit/core/plugin';
import { Icon } from 'unidit/core/ui';
import { css, position, refs, trim } from 'unidit/core/helpers';
...
Config.prototype.controls.find = {
	tooltip: 'Find',
	icon: 'search',
	exec(unidit: IUnidit, _, { control }) {
		const value = control.args && control.args[0];

		switch (value) {
			case 'findPrevious':
				unidit.e.fire('searchPrevious');
				break;

			case 'findNext':
				unidit.e.fire('searchNext');
				break;

			case 'replace':
				unidit.execCommand('openReplaceDialog');
				break;

			default:
				unidit.execCommand('openSearchDialog');
		}
	},
...
export class search extends Plugin {
	...
	private template = `<div class="unidit-search">
			<div class="unidit-search__box">
				<div class="unidit-search__inputs">
				...
				<div class="unidit-search__counts">
					<span data-ref="counter-box">0/0</span>
				</div>
				<div class="unidit-search__buttons">
					...
					<button data-ref="replace-btn" tabindex="0" type="button" class="unidit-ui-button">${this.j.i18n(
						'Replace'
					)}</button>
				</div>
			</div>
	...
	open = (searchAndReplace: boolean = false): void => {
		if (!this.isOpened) {
			this.searchBox.classList.add('unidit-search_active');
			this.isOpened = true;
		}
	...
	close = (): void => {
		...
		this.searchBox.classList.remove('unidit-search_active');
		this.isOpened = false;
	};

	/** @override */
	afterInit(editor: IUnidit): void {
	...
	private calcSticky(enabled: boolean): void {
		if (this.isOpened) {
			this.searchBox.classList.toggle('unidit-search_sticky', enabled);
