...
export class resizeCells extends Plugin {
	...
	private createResizeHandle = () => {
		if (!this.resizeHandler) {
			this.resizeHandler = this.j.c.div('unidit-table-resizer');
	...
	private onHandleMouseDown(event: MouseEvent) {
		...
		this.j.lock(key);
		this.resizeHandler.classList.add('unidit-table-resizer_moved');
	...
	private onMouseUp(e: MouseEvent): void {
		...
		this.resizeHandler.classList.remove('unidit-table-resizer_moved');
	...
	beforeDestruct(unidit: IUnidit): void {
		if (unidit.events) {
			unidit.e.off(this.j.ow, '.resize-cells');
			unidit.e.off('.resize-cells');
		}
	}
}
