...
import { $$, css, scrollIntoViewIfNeeded } from 'unidit/core/helpers';
...
Config.prototype.controls.table = {
	...
	popup: (editor: IUnidit, current, control, close, button) => {
		...
		const generateExtraClasses = (): string => {
			...
			if (control.data) {
				...
				Object.keys(classList).forEach((classes: string) => {
					out.push(
						`<label class="unidit_vertical_middle"><input class="unidit-checkbox" value="${classes}" type="checkbox"/>${classList[classes]}</label>`
					);
				});
			}
			return out.join('');
		...
		const form: HTMLFormElement = editor.c.fromHTML(
				'<form class="unidit-form unidit-form__inserter">' +
					'<div class="unidit-form__table-creator-box">' +
					'<div class="unidit-form__container"></div>' +
					'<div class="unidit-form__options">' +
					generateExtraClasses() +
					'</div>' +
					'</div>' +
					'<label class="unidit-form__center">' +
			...
			blocksContainer = form.querySelector(
				'.unidit-form__container'
			) as HTMLDivElement,
			options = form.querySelector(
				'.unidit-form__options'
			) as HTMLDivElement,
			cells: HTMLElement[] = [];
		...
		const mouseenter = (e: MouseEvent, index?: number): void => {
			...
			for (let i = 0; i < cells.length; i += 1) {
				if (
					...
					cells[i].className = 'unidit_hovered';
				} else {
					cells[i].className = '';
				}
			}
