...
import { css, defaultLanguage, attr, callPromise } from 'unidit/core/helpers/';
import { error } from 'unidit/core/helpers';
import { MODE_SOURCE } from 'unidit/core/constants';
...
/**
 * Base URL where the root directory for {@link Unidit.defaultOptions.iframe|iframe} mode
 *
 * @example
 * ```javascript
 * new Unidit('#editor', {
 *    iframe: true,
 *    iframeBaseUrl: 'http://nazrul.dev/pacakges/unidit/docs/',
 * });
 * ```
 */
 ...
 */
Config.prototype.iframeTitle = 'Unidit Editor';
 ...
 * new Unidit('#editor', {
 *    iframe: true,
 *    iframeDefaultSrc: 'http://nazrul.dev/pacakges/unidit/docs/',
 * });
 * ```
 */
...
Config.prototype.iframeStyle =
	...
	'margin-top:0;' +
	'}' +
	'.unidit_editor .unidit_iframe_wrapper{' +
	'display: block;' +
	'clear: both;' +
	'user-select: none;' +
	'position: relative;' +
	'}' +
	'.unidit_editor .unidit_iframe_wrapper:after {' +
	'position:absolute;' +
	'content:"";' +
	'z-index:1;' +
	'top:0;' +
 ...
 */
export function iframe(editor: IUnidit): void {
	...
	editor.e
		...
		.on(
			...
			(__doc: Document | undefined, unidit: IUnidit) => {
				const doc =
					__doc ||
					(
						(unidit.iframe as HTMLIFrameElement)
				...
				doc.write(
					opt.iframeDoctype +
						`<html dir="${
							opt.direction
						}" class="unidit" lang="${defaultLanguage(
							opt.language
						)}">` +
						'<head>' +
						...
						(opt.iframeBaseUrl
							...
							: '') +
						'</head>' +
						'<body class="unidit-wysiwyg"></body>' +
						'</html>'
				);
		...
		.on('createEditor', (): void | Promise<void> | false => {
			...
			iframe.className = 'unidit-wysiwyg_iframe';
						...
						html = html
							...
							.replace(
								...
								body[0]
									...
									.replace(
										/<(style|script|span)[^>]+unidit[^>]+>.*?<\/\1>/g,
										''
									)
							)
							.replace(
								/(class\s*=\s*)(['"])([^"']*)(unidit-wysiwyg|unidit)([^"']*\2)/g,
								'$1$2$3$5'
							)
