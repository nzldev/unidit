...
import { attr, isString, isFunction } from 'unidit/core/helpers';
import { Icon } from 'unidit/core/ui';
import { UIList } from 'unidit/core/ui';
import { autobind, component, watch } from 'unidit/core/decorators';
...
export class UIButton extends UIElement implements IUIButton {
	...
	protected onChangeText(): void {
		this.text.textContent = this.unidit.i18n(this.state.text);
	}
	...
	constructor(unidit: IViewBased, state?: IUIButtonStatePartial) {
		super(unidit);
...
export function Button(unidit: IViewBased, icon: string): IUIButton;
...
export function Button(
	...
	const button = new UIButton(unidit);
