...
import { eventEmitter, getContainer } from 'unidit/core/global';
import { UIElement } from 'unidit/core/ui';
import { autobind, throttle } from 'unidit/core/decorators';
...
export class Popup extends UIElement implements IPopup {
	...
	open(getBound: getBoundFunc, keepPosition: boolean = false): this {
		markOwner(this.unidit, this.container);
		...
		const parentContainer = getContainer(this.unidit, Popup);
	...
	private calculatePosition(
		...
		// Try find match position inside Unidit.container
	...
	constructor(unidit: IViewBased, readonly smart: boolean = true) {
		super(unidit);
