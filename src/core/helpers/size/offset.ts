...
import type { IBound, IHasScroll, IUnidit, IViewBased } from 'unidit/types';
...
export const offset = (
	...
	const iframe = (unidit as IUnidit).iframe;

	if (!recurse && unidit && unidit.options && unidit.o.iframe && iframe) {
		const { top, left } = offset(iframe, unidit, unidit.od, true);
