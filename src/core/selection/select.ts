...
export class Select implements ISelect {
	constructor(readonly unidit: IUnidit) {
		unidit.e.on('removeMarkers', () => {
			this.removeMarkers();
		});
	}
	...
	/**
	 * Return current work place - for Unidit is Editor
	 */
	 ...
	 * console.log(Unidit.modules.Helpers.stripTags(editor.s.html)); // plain text
	 * ```
	 */
	get html(): string {
		const sel = this.sel;
	...
	*wrapInTagGen(): Generator<HTMLElement> {
		if (this.isCollapsed()) {
			const font = this.unidit.createInside.element(
				'font',
				INVISIBLE_SPACE
			);
		...
		// fix issue https://github.com/nzldev/unidit/issues/65
