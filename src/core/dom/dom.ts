...
import { Select } from 'unidit/core/selection';
import { TEMP_ATTR } from 'unidit/core/constants';
...
export class Dom {
	 ...
	 * @example
	 * ```javascript
	 * Unidit.modules.Dom.replace(parent.editor.getElementsByTagName('span')[0], 'p');
	 ...
	 * @example
	 * ```javascript
	 * Unidit.modules.Dom.each(parent.s.current(), function (node) {
	 *  if (node.nodeType === Node.TEXT_NODE) {
	 *      node.nodeValue = node.nodeValue.replace(Unidit.INVISIBLE_SPACE_REG_EX, '') // remove all of
