...
/**
 * Class Unidit. Main class
 */
export class Unidit extends ViewWithToolbar implements IUnidit {
	/** @override */
	className(): string {
		return 'Unidit';
	}
	 ...
	 * await unidit.waitForReady();
	 * unidit.e.fire('someAsyncLoadedPluginEvent', (test) => {
	 *   alert(test);
	 * });
	 * ```
	 */
	...
	/**
	 * Define if object is Unidit
	 */
	override readonly isUnidit: true = true;
	...
	/**
	 * Factory for creating Unidit instance
	 */
	static make(element: HTMLElement | string, options?: object): Unidit {
		return new Unidit(element, options);
	}
	...
	static instances: IDictionary<IUnidit> = instances;
	...
	private __defaultStyleDisplayKey = 'data-unidit-default-style-display';
	private __defaultClassesKey = 'data-unidit-default-classes';

	private commands: IDictionary<Array<CustomCommand<IUnidit>>> = {};
	...
	/**
	 * options All Unidit settings default + second arguments of constructor
	 */
	...
	get filebrowser(): IFileBrowser {
		...
		const options = ConfigProto(
			{
				defaultTimeout: unidit.defaultTimeout,
				uploader: unidit.o.uploader,
				language: unidit.o.language,
				license: unidit.o.license,
				theme: unidit.o.theme,
				defaultCallback(data: IFileBrowserCallBackData): void {
					if (data.files && data.files.length) {
						data.files.forEach((file, i) => {
							...
							if (isImage) {
								unidit.s.insertImage(
									url,
									null,
									unidit.o.imageDefaultWidth
								);
							} else {
								unidit.s.insertNode(
									unidit.createInside.fromHTML(
		...
		return unidit.getInstance<IFileBrowser>('FileBrowser', options);
	}
	...
	getEditorValue(
		...
		if (removeSelectionMarkers) {
			value = value.replace(
				/<span[^>]+id="unidit-selection_marker_[^>]+><\/span>/g,
				''
			);
		}
	 ...
	 * @example
	 * ```javascript
	 * var unidit = new Unidit('#editor);
	 *
	 * unidit.setEditorValue('test test test');
	 *
	 * unidit.registerCommand('replaceString', function (command, needle, replace) {
	 *      var value = this.getEditorValue();
	 *      this.setEditorValue(value.replace(needle, replace));
	 *      return false; // stop execute native command
	 * });
	 *
	 * unidit.execCommand('replaceString', 'test', 'stop');
	 *
	 * console.log(unidit.value); // stop test test
	 *
	 * // and you can add hotkeys for command
	 * unidit.registerCommand('replaceString', {
	 *    hotkeys: 'ctrl+r',
	...
	private execCustomCommands(
		...
		if (this.commands[commandName] !== undefined) {
			...
			const exec = (command: CustomCommand<Unidit>) => {
				let callback: ExecCommandCallback<Unidit>;
	...
	override lock(name = 'any'): boolean {
		if (super.lock(name)) {
			...
			this.s.clear();
			this.editor.classList.add('unidit_disabled');
	...
	override unlock(): boolean {
		if (super.unlock()) {
			this.editor.classList.remove('unidit_disabled');
	...
	/**
	 * Return current editor mode: Unidit.MODE_WYSIWYG, Unidit.MODE_SOURCE or Unidit.MODE_SPLIT
	 */
	getMode(): Modes {
		return this.mode;
	}
	...
	setMode(mode: number | string): void {
		 ...
		 *     data.mode = Unidit.MODE_SOURCE; // not respond to the mode change. Always make the source code mode
		 * });
		 * ```
		 */
		...
		if (this.o.saveModeInStorage) {
			this.storage.set('unidit_default_mode', this.mode);
		}
	...
	setDisabled(isDisabled: boolean): void {
		...
		if (this.editor) {
			...
			this.container.classList.toggle('unidit_disabled', isDisabled);
	...
	addPlace(
		...
		let container = this.c.div('unidit-container');

		container.classList.add('unidit');
		container.classList.add('unidit-container');
		container.classList.add(`unidit_theme_${this.o.theme || 'default'}`);
		...
		if (this.o.inline) {
			...
			container.classList.add('unidit_inline');
			container.classList.add('unidit-container');
		}
		...
		const workplace = this.c.div('unidit-workplace', {
		...
		const editor = this.c.div('unidit-wysiwyg', {
	...
	private initEditor(buffer: null | string): void | Promise<any> {
		...
		return callPromise(result, () => {
			...
			if (this.o.saveModeInStorage) {
				const localMode = this.storage.get('unidit_default_mode');
	...
	/**
	 * Unidit's Destructor. Remove editor, and return source input
	 */
